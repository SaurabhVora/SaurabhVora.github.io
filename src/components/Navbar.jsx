        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href} 
              className="text-neutral-400 hover:text-primary text-sm font-medium tracking-wide transition-all duration-300 relative group py-2"
            >
              {link.name}
          ...
          <a 
            href="#contact" 
            className="bg-primary/10 border border-primary/30 hover:bg-primary hover:text-darker text-primary text-xs font-bold px-5 py-2.5 rounded-full tracking-wider transition-all duration-300"
          >
            HIRE ME
          </a>
        </nav>
            ...
            <nav className="flex flex-col px-6 py-6 gap-4">
              {navLinks.map((link) => (
                <a 
                  ...
                  className="text-neutral-400 hover:text-primary text-base font-medium py-2 border-b border-white/5 last:border-0 transition-colors"
                >
                  {link.name}
                </a>
              ))}
